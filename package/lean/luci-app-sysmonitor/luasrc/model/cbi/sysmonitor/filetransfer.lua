...
function IsUpdateFile(name)
	name = name or ""
	local ext = string.lower(string.sub(name, -28, -1))
	local extn1 = string.lower(string.sub(name, 17, 26))
	return extn1 == "phicomm-n1" and ext == "aarch64-openwrt-firmware.zip"
end

function IsIMGgzFile(name)
	name = name or ""
	local ext = string.lower(string.sub(name, -31, -1))
	local extn1 = string.lower(string.sub(name, 17, 26))
	return extn1 == "phicomm-n1" and ext == "aarch64-openwrt-firmware.img.gz"
end
...
function IsIMGFile(name)
	...
	local ext = string.lower(string.sub(name, -28, -1))
	local extn1 = string.lower(string.sub(name, 17, 26))
	return extn1 == "phicomm-n1" and ext == "aarch64-openwrt-firmware.img"
end
...
btnis.render = function(self, section, scope)
	...
	elseif IsUpdateFile(inits[section].name) then
		scope.display = ""
	elseif IsIMGFile(inits[section].name) then
		scope.display = ""
	elseif IsIMGgzFile(inits[section].name) then
		scope.display = ""
	else
		scope.display = "none"
	end
...
btnis.write = function(self, section)
	if Ispasswall(inits[section].name) or Isshadowsocksr(inits[section].name) then
		...
		luci.sys.exec("echo 'Update "..inits[section].name.."' >/var/log/sysmonitor.log")
		luci.sys.exec("echo '------------------------------------------------------------------------------------------------------' >>/var/log/sysmonitor.log")
		luci.sys.exec("mv /tmp/upload/"..inits[section].name.." /tmp/upgrade")	
		luci.http.redirect(luci.dispatcher.build_url("admin", "sys", "sysmonitor","log"))
		luci.sys.exec("n1-update no &")
	end
end
